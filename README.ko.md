...
나란히 놓고, **Submit for review** 는 초록 심사 플랜 · 빌드 번호(다시 타이핑하거나, **빌드 번호: 자동**
토글이면 플랜이 스토어에서 읽은 번호 그대로) · Google Play 를
골랐으면 이번 제출의 관리형 게시 체크가 있어야 열린다 — 어떤 상태에도 Release · Publish · Rollout
버튼은 없고, `unsafe_release_type` 판정은 닫을 수 없는 빨간 띠가 된다. 회차가 도는 동안은 화면
맨 위의 큰 막대가 버전 · 빌드 번호 · 지금 단계(`S5 시나리오 QA · 9단계 중 6번째`) · 전체 % · 예상 완료를
보이고, 마우스를 올리면 근거까지 전부 보인다. 프로파일에 드라이버가 있으면
...
`confirm` 을 보내고(**빌드 번호: 자동** 이면 서버가 플랜의 번호로 스스로 확인해 회차가 멈추지 않는다),
**Rehearsal (no upload)** 버튼은 업로드 프리셋을 예행으로만 돌린다 — 진짜 업로드는
