...
- `tools/guard_production.py` 훅(`.claude/settings.json`)이 막아 준다 — 운영 체크아웃·설정·데이터를
  고치거나 **서비스 venv 밖의 빌드로 운영 DB 를 열면**(`rcm token`·`serve`·`worker` — 운영 설정,
  그 **사본**, `RCM_SERVER_DATA_DIR` 어느 쪽으로든 유효 `data_dir` 이 운영이면; 다른 빌드는 열면서
  마이그레이션한다) 거부되고, 배포(서비스 venv 설치 · 재시작)는 물어본다. 서비스 venv 는 PATH 의
  `rcm` 이 아니라 editable 설치가 본 체크아웃을 가리키는 venv 다(워크트리 `.venv` 를 활성화한
  셸에서도 같다). `rcm gc --dry-run --config` 는 사본 위에서 돌아 허용이다.
