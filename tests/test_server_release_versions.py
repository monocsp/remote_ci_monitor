...
명세: docs/version-page-workplan.md §2.2 · §7 AC-B2~B9 · AC-B11 · §8 E1 · E3~E6 · E9~E11 · E14 ·
E15 · E25.
...
- 409 표: `version_exists` · `version_closed` · `version_running` · `setup_incomplete` ·
  `listing_json_unsupported`; 400 은 `empty` · `pattern` · `not_greater` · `listing_key` ·
  `listing_too_large` · `build_name_mismatch`.
...
REVIEW_LISTING_PRESET = preset(
    ...
    [*REVIEW_INPUTS, {"name": "listing_json", "type": "string", "default": ""}],
)
...
def test_upload_forwards_the_edited_listing_too(vsrv):
    ...
    upload_listing = preset(
        ...
        [
            ...
            {"name": "listing_json", "type": "string", "default": ""},
        ],
    )
    srv.cfg.presets = (parse_preset(upload_listing), *srv.cfg.presets)
    srv.set_profile(
...
def test_a_round_started_with_build_number_auto_keeps_the_version_on_the_confirm_run(vsrv):
    ...
    assert "--version-id 1" in Path(second["log_path"]).read_text().splitlines()[-3]
    deadline = 100
    while srv.store.get_version(vid)["state"] == "running" and deadline:
        deadline -= 1
        __import__("time").sleep(0.05)
