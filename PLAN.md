...
- 실패 스텝: `step-end::fail` 이 찍힌 스텝이 `failed_step` 이고 그건 **확정**이다(`failed_step_guessed: false`). 그 마커가 없는데 종료 코드가 ≠ 0 이면 마지막 스텝을 고르되 **추측으로 표시한다**(`failed_step_guessed: true`). 이름을 아예 안 대는 것보다는 낫지만 사실인 척하면 무죄인 스텝을 범인으로 만든다 — 2026-09-08 운영에서 실패 55건 중 16건이 그랬다(함정 #7). 스키마 v10 앞에 끝난 잡은 `null`(모른다).
- 함정과 테스트(픽스처로 잠근다):
