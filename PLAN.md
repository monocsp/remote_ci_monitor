...
- **이유 `reason`**: 잡이 지금 왜 이 상태인지 서버가 계산해 싣는 **단일 표시 사유**다(`estimate.overdue`·`estimate.stuck`·`estimate.quiet` 은 근거 플래그로 따로 싣는다). `running` · `waiting_for_lane`(`ahead_job_id` = 가장 먼저 비는 레인의 잡) · `blocked_by_group` · `uploading` · `upload_stalled`(`upload_stall_seconds` 동안 바이트가 안 옴) · `materializing` · `overdue` · `stuck` · `quiet`(출력만 조용하다 — 아래 「stuck」 참조) · `cancelling` · `paused` · `not_scheduled`(대기 잡이 있고 정지 아니고 그룹에 안 막혔는데 idle 레인이 있는 채로 `max(워커 since, 잡 queued_at)` 부터 10초가 넘음 — 스케줄러 이상) · `worker_down`(모든 레인 다운) · `held_by_load`(집었을 레인이 호스트 부하로 보류 중 — M5f). 행동 가능한 이유(`worker_down` → `stuck` → `upload_stalled` → `not_scheduled` → `blocked_by_group` → `overdue` → `paused`)는 화면 요약 「Not moving」에 이 순서로 오른다. **`held_by_load` 와 `quiet` 은 여기 안 올린다** — `held_by_load` 는 의도된·자가 치유되는 상태라 `paused` 와 같은 종류이고(오너 결정 45), `quiet` 은 사람이 손쓸 것이 없는 **관찰**이다(출력이 조용한 것뿐이고, 죽었다는 주장이 아니다 — 올리면 오늘의 빨간 소음이 이름만 바꿔 남는다). 오래 닫혀 있으면 `rcm check` 가 경고한다.
...
- **stuck · quiet**: `running` 이고 아래 셋 중 하나면 `estimate.stuck: true` 이고, 어느 근거가 발동했는지를 `estimate.stuck_code` 로 함께 싣는다 — 화면은 **발동한 근거만** 적는다(예전엔 언제나 「n배」를 붙여 「멈춘 듯 · 예상의 1배」가 나왔다).
  1. `over_step` — 지금 도는 **단계**가 자기 실측 중앙값의 `step_stuck_multiplier(3)` 배를 넘었다. 중앙값은 같은 key 의 최근 **성공** 잡들이 이미 찍어 둔 단계 마커에서 뽑고, `step_min_samples(3)` 개 이상 모인 단계에만 쓴다. 임계에는 하한이 있다: `max(step_stuck_multiplier × 단계중앙값, no_output_seconds)` — 중앙값 0.5초짜리 단계를 1.5초 만에 죽었다고 부르는 것은 침묵 규칙이 낼 수 없던 주장이다. `estimate.step_expected_seconds` 로 그 중앙값을 싣는다.
  2. `over_elapsed` — `elapsed > stuck_multiplier(3) × expected`. 예전 그대로다.
  3. `no_output` — **단계 마커를 아예 안 찍는 잡**만. 그런 잡은 침묵이 우리가 가진 유일한 신호라 `now − progress.last_output_at > no_output_seconds(240)` 이 곧 판정이다.
  단계는 넘어가는데 출력만 조용한 잡은 `stuck` 이 아니라 **`quiet`**(`estimate.quiet: true`) — 회색, 경보 아님. 실측이 아직 없는 새 key 도 여기로 떨어진다(모르는 것은 모르는 것이지 죽은 것이 아니다. 진짜로 죽은 잡은 `over_elapsed` 와 프리셋의 `timeout_seconds` 가 잡는다). `overdue` 와도 다르다(overdue 는 「늦다」, stuck 은 「죽었을지 모른다」, quiet 은 「말이 없다」). 키 셋(`quiet`·`stuck_code`·`step_expected_seconds`)과 `reason` 값 하나를 **더하기만** 했으므로 `schema_version` 은 1 그대로다.
...
| `GET /jobs/{id}?tail=N` | 없음(`log_tail` 은 토큰) | 잡 스냅샷(활성 잡은 queue 행, 종료 잡은 recent 행 모양 — recent 행엔 `log_tail` 키가 없다). **`failed`·`timed_out` 잡에만 `failures[]`·`failures_truncated` 를 더 싣는다**(M5h · 결정 67 — 이 이력 질의는 여기에만 있고 `/api/status` 에는 없다). **종료 잡에는 `step_timeline`** — 저장된 마커로 다시 낸 스텝별 시간(`timing`·`steps_total`·`steps_total_partial`·`steps[]{index,name,started_at,ended_at,seconds,ok}`). 마커가 없으면 `steps: []`, 마커 조회가 깨지면 `null` + `step_timeline_error_code`(fail-open 금지). 재구성한 `failed_step` 은 싣지 않는다(M5j G1 · `docs/gate-optimization-workplan.md` 결정 84). `rcm wait`·`rcm run` 의 최종 JSON 은 이 문서의 복사라 같은 키를 갖는다 — 계약이다. `log_tail` 은 **유효 토큰(그 잡의·합류자·admin) 요청이고 `running`/`cancelling` 일 때만** 싣고 아니면 null. `tail` 기본 5줄, 잡당 8KiB 상한, `rcm wait` 는 `tail=0` |
...
| `GET /api/repos/{repo}/release` | `read_auth`(게이트 무관) | 릴리스 화면의 상태 — `setup` · 역할별 최신(`plan{job_id,state,measured_at,age_seconds,stale,build_name,doc}` · `review{plan,result}` · `upload`) · `jobs[]`(역할 프리셋의 잡, 새것부터 ≤ 50). 최신 = 그 프리셋 잡 중 묶음에 역할 파일(`plan.json` · `upload.json` · `review-plan.json`/`review.json`, basename)이 있는 가장 새 잡, `doc` 은 묶음에서 읽은 JSON(≤ 256 KB · 못 읽으면 `null`+`doc_error`). `stale` = `plan_max_age_minutes` 초과. DB 와 산출물 저장소만 본다 |
| `POST /api/repos/{repo}/release/plan` · `/review` · `/upload` | 토큰(`review mode=submit` · `upload mode=upload` 는 admin) | 역할 프리셋을 정규 `POST /jobs` 경로(git_ref · 합류 · 우선순위 · 풀)로 제출 → 202 `{job_id, joined, state, sha}`. 입력은 계약 §2 의 것만. 게이트 닫힘 409 `setup_incomplete`. 서버가 지키는 문(UI 무관): `review_plan_required`·`review_plan_stale`·`review_plan_blocked`(submit 은 같은 build 의 성공한 · 안 오래된 · `plan_verdict == ok` 심사 계획) · `managed_publishing_unconfirmed`(android 포함 submit 은 이 본문의 `confirmed-on`) · `plan_required`·`plan_stale`·`build_number_mismatch`(친 번호 == 계획의 `n`, 대신 채우지 않는다) · `unsafe_preset`(`automatic_release`·`rollout`·`release_status` 입력 선언) · `role_not_configured`. 정본은 `docs/configuration.md` 「Release routes and the driver」 |
| `GET /api/repos/{repo}/release/listing[?build_name=]` · `/listing/file?path=` · `POST …/listing/validate` | `read_auth` · `read_auth` · 토큰 | 미러의 default_branch 체크아웃(`<data_dir>/listing/<repo>/`, sha 바뀌면 다시)에서 프로파일의 `preview`·`diff`(20초 · 64 KB) → `{configured, sha, preview[], diff[], release_notes, screenshots[], errors[]}`. `file` 은 스크린샷 글롭에 맞는 이미지만 ≤ 5 MB. `validate` 는 `{version}`·`{build}` 치환 → `{ok, lines[], exit}` |
| `GET /api/repos/{repo}/release/github` | `read_auth` | 미러에서만: `log[]`(default_branch 최근 5) · `tags[]`(태그 패턴 접두사, 최근 5) · `prs: null`(다음: GH_TOKEN 비밀로) |
| `POST /api/repos/{repo}/release/start` · `/confirm` · `/abort` · `/retry` · `GET …/release/driver` | admin · admin · admin · admin · `read_auth` | 프로파일의 `driver` 를 `<data_dir>/driver/<repo>/checkout` 에서 **분리된** 자식으로(`--build-name X [--android-track T] [--dry-run]` · `--confirm-build-number N` · `--abort` · `--retry`). env = 잡 env(비밀 폴더) + `RCM_SERVER` + `RCM_TOKEN`(그 실행에 발급한 client 토큰 `store-driver:<repo>:<id>`, 끝나면 폐기). 로그 `<data_dir>/driver/<repo>/<build_name>.log` 에 덧붙임. 한 저장소에 실행 하나(409 `release_running`). `confirm` 은 로그 마지막 `plan: N = <n>` == 친 번호일 때만(409 `build_number_mismatch`). 대장은 DB `releases` 표(v18, gc 무관). `GET driver` 는 `{configured, running, release_id, kind, build_name, started_at, started_by, pid, exit_code, confirmed_n, log_tail[60], plan_n, status[](`--status` 10초 동기), status_error}` — pid 가 죽은 실행은 여기서 닫고 토큰을 폐기한다 |
...
stuck_multiplier = 3                # elapsed > 3 × expected → stuck (over_elapsed)
no_output_seconds = 240             # 이만큼 조용하면 → quiet. 단계 임계의 하한이자,
                                    # 단계를 아예 안 찍는 잡의 마지막 근거(no_output)
step_stuck_multiplier = 3           # 지금 단계 > 3 × 그 단계의 실측 중앙값 → stuck (over_step)
step_min_samples = 3                # 단계 중앙값을 믿기 시작하는 성공 표본 수
...
**배치·상태·문구·표기 규칙은 `docs/wireframes/web-queue.html` 이 정본이다**(기획 항목 35개 + 「4. 이 화면이 정한 규칙」). 요지: 큐 표 **위에** 요약 세 칸 — **Your jobs**(요청자·합류자 기준 내 잡의 순번·ETA) · **Needs a look**(행동 가능한 이유만, `worker_down → stuck → upload_stalled → not_scheduled → blocked_by_group → overdue → paused` 순 — 2026-09-15 이전 이름은 「Not moving」) · **Host pressure**(CPU·Mem·GPU·load 한 줄) — 를 먼저 보이고, 그 아래 큐 표(Job · Job name · Requested by · **Status** · Elapsed · Finishes+신뢰도 · Source), 호스트 카드(구간 막대 + 5분 sparkline), 최근 완료 `recent_count` 건, Estimates(접힘). 변형 상태 19개(빈 큐 ≠ 조회 실패, 연결 끊김, stale, 초과, stuck, 워커 다운, 정지, 토큰 거부, 취소 중, 업로드 멈춤, 워크스페이스 준비, …)는 목업의 2절.
...
- **호스트 절**: 접힌 채로 뜨고, 화면이 스스로 펼치는 것은 **사람이 손대야 하는 것**뿐이다 — 표본 stale · `hosts_error` · 남은 디스크 10 GiB 미만. **부하로는 펼치지 않고**(빌드 머신의 CPU 85% 는 이상이 아니라 일하는 중이라는 뜻이다) **스스로 닫지도 않는다**(닫는 것은 사람의 일이다). 사람의 선택은 `rcm.host` 에 남아 그 뒤로는 그것이 이긴다. 압력 판정 `busy` 에는 이력이 있다 — 85% 에서 켜지고 전부 80% 아래로 내려와야 꺼진다(숫자의 황토는 그대로 85% 기준). 2026-09-14 실측: 렌더마다 열림 상태를 판정으로 다시 쓰던 탓에 2분에 세 번 여닫히며 그 아래가 315px 씩 뛰었다(CLS 0.245).
...
- **전체 진행 막대**(2026-09-09 오너 요청 · 같은 날 Codex 리뷰 반영): 도는 행의 **「진행 시간」 칸 안** 84px 막대(별도 행이 아니다 — 2026-09-15 시각 위계 개정). 축이 둘이다 — **근거**(`basis`)와 **형편**(`condition`). 근거는 `steps`(잡이 `::rcm::steps::N` 으로 선언한 총계 → `62% · 단계 5/8`) · `time`(추정 소요 대비 경과 → `62% · 지난 실행 기준`·`62% · 설정한 예상 시간 기준`, 1초마다 스스로 자란다) · `none`. 형편은 `preparing`(준비 중) · `stuck`(응답 없음) · `over`(예상 시간 초과) · `finalizing`(선언한 단계를 다 끝냈는데 안 끝남) · `quiet`(조용함) 이고 라벨에 ` · ` 로 붙는다. 우선순위는 `preparing > stuck > over > finalizing > quiet > normal` — **`stuck` 이 `quiet` 을 이긴다**(경보를 먹는 쪽은 fail-open 이라 금지).
