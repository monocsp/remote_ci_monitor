...
- **전체 진행 막대**(2026-09-09 오너 요청): 도는 행 아래 한 줄. 눈금의 **근거를 글자로 함께** 준다 — 잡이 `::rcm::steps::N` 으로 총계를 선언했으면 `62% · 스텝 5/8`, 아니면 추정 소요 대비 경과로 `62% · 예상 시간 기준`(시간 눈금은 1초마다 스스로 자란다). `steps_total_partial`(총계를 세는 중)은 근거가 아니고, 추정을 넘긴 잡은 `예상 시간 초과`(빗금), stuck·준비 중·근거 없음은 `진행률 —`(눈금 없는 빗금) — 모르는 값을 0% 로 그리지 않는다.
