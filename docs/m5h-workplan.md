...
| 이름을 안 쓰는 팀에게 **런타임 안내** | 실패할 때마다 「`::rcm::fail::` 을 찍으면 이력이 붙습니다」를 찍으면 그건 매 실패마다 붙는 잔소리다. 문서 · CHANGELOG · `examples/session/ci-gate.sh` 가 말한다 (검증 라운드 9) |
| 취소한 잡에 「어디였나」 남기기 | 결정 64 다. 실기 검증자는 「취소한 잡에 가장 자주 묻는 질문이 그것」이라고 지적했다 — 뒤집으려면 결정 64 를 고친다(§4.1 각주). 지금은 로그가 답한다 |
