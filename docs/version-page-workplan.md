...
## 16. C 단계 격리 검증이 찾은 것 (2026-09-20)

AC-C1~C10 을 살아 있는 서버와 브라우저로 확인하고, 체크리스트 밖에서 열이 나왔다. 1 은 **웹이 아니라
서버**였고 이 PR(#169)에서 고쳤다. 3·4·8 은 바텀시트 PR 이 가져갔고, 2·5·6·7·9 는 그다음 웹 PR 이다.
10 은 소유자에게 물을 것이 남았다.

| # | 무엇 | 왜 아픈가 | 할 일 |
|---|---|---|---|
| 1 | **401 이 요청 본문을 안 읽고 답한다** | 본문을 실은 요청이 401 을 받으면 서버는 그 본문을 한 바이트도 안 읽고 나간다. keep-alive 연결이 본문 한가운데 멈춰 있으니 **다음 요청이 남은 바이트 안에서 파싱된다**. 생 소켓 재현: `PUT …/release/versions/2/listing`(본문 28바이트) → `401`, 이어진 `GET /api/health` → `501 Unsupported method ('{"ios":{"subtitle":"abcde"}}GET')`. 같은 요청이 403·200 이면 멀쩡했다 — 그 라우트의 403·200 은 본문을 먼저 읽는 길이라서다. 고치면서 보니 401 만의 병이 아니었다: **본문을 읽기 전에 거절하는 길이면 다 같다**(admin 아닌 토큰의 `POST /pause` 는 403 인데도 같은 어긋남이 났다). 화면에서는 토큰이 편집 중에 죽어 저장이 401 로 깨끗이 실패한 뒤, «다시 저장» 이 401 이 아니라 400 이나 501 HTML 오류 쪽지로 돌아온다 | **고쳤다(이 PR)** — 서버는 어떤 상태 코드로 답하든 선언된 본문을 먼저 먹는다. 못 먹는 본문(길이를 모르는 chunked · 상한 1 MB 초과)만 안 읽고 `Connection: close` 로 나간다. 411·413·415 의 뜻은 그대로다 |
| 2 | `refusalText` 가 JSON 이 아닌 오류 본문을 안 자른다 | 1 번 때문에 돌아온 HTML 오류 페이지가 그대로 저장 배지에 들어가 500자쯤이 한 줄로 깔렸다. 길이 제한이 없어 어떤 본문이든 화면을 밀어낸다 | 길이 상한을 두고 «HTML 이면 상태 코드만» 으로 줄인다 *(웹 — 바텀시트 PR 다음)* |
| 3 | App Store 버전이 있는 드래프트를 버리면 목록 행이 28초쯤 그대로다 | 버리기가 스토어 삭제 잡을 내보내는데 행 모델이 `delete_job_id` 를 아예 안 본다. 그동안 행은 «편집 중» 이라 버튼이 살아 있고, 다시 눌러도 되는 것처럼 보인다 | 행 모델이 `delete_job_id` 를 «진행 중» 조건에 넣는다 *(웹 — 바텀시트 에이전트에게 넘김)* |
| 4 | `#/store/<r>/status` 가 아직 심사 패널을 그린다 | 제출 버튼과 체크박스가 그대로 있다. §3.2 는 이 화면에서 빼기로 했고 R9 는 심사 조작이 바텀시트에만 있어야 한다고 말한다 — 같은 일을 두 곳에서 할 수 있으면 어느 쪽이 정본인지 아무도 모른다 | 바텀시트 PR 이 조작 부분을 걷어낸다. AC-D6 이 확인한다 |
| 5 | 한국어 문자열 둘이 `withJosa` 를 건너뛴다 | `version.edit.readonly.closed` 가 «제출됨 라» 로 나온다(«제출됨이라» 여야 한다). `store.gate.return` 은 으로/로 를 안 고른다 — `JOSA_PAIRS` 에 그 짝이 없다 | 두 문자열을 `withJosa` 로 보내고 으로/로 짝을 `JOSA_PAIRS` 에 넣는다 *(웹)* |
| 6 | «파일에서» 가 **계획의** 버전을 읽는다 | 1.2.0 드래프트에 1.1.1 의 릴리스 노트가 «파일에서» 라는 꼬리표를 달고 떴다. `GET …/release/listing` 은 `build_name` 을 받는데 페이지가 안 준다. 남의 버전 문안을 자기 것이라 말하는 셈이라 그대로 제출될 수 있다 | 페이지가 드래프트 자신의 이름을 `build_name` 으로 넘긴다 *(웹)* |
| 7 | «이전 버전과 같음» 스크린샷 칩이 아무것도 재지 않는다 | 프리필에 `screenshots` 가 아예 없어도 뜬다. 재지 않고 같다고 말하는 문구는 「번호를 지어내지 않는다」에 정면으로 어긋난다 | 문구를 사실로 바꾼다 — «rcm 은 스크린샷을 건드리지 않습니다» *(웹)* |
| 8 | «진행 중» 이 비활성 버튼의 `title` 로만 자기를 설명한다 | 왜 못 누르는지가 마우스를 얹어야 나온다. 터치에는 그 길이 없다 | 사유를 화면에 한 줄로 적는다 *(웹 — 바텀시트 에이전트에게 넘김)* |
| 9 | «입력한 값은 그대로 둔다» 는 문구가 실제보다 넓다 | 지켜 주는 것은 **사람이 건드린 칸**뿐이고, 손대지 않은 칸은 원격 값을 칩과 함께 받아들인다. 둘 다 옳은 동작이지만 문구가 그 차이를 안 말한다 | 문구만 고친다 *(웹)* |
| 10 | 버전 페이지에 «버리기» 가 없다 | 드래프트를 버리는 길이 목록 행 하나뿐이다. §15 가 두 곳을 뜻했는지가 분명하지 않다 | 소유자에게 묻는다 — 두 곳이면 버전 페이지에도 같은 확인 대화상자를 둔다 |

## 17. D 단계 격리 검증이 찾은 것 (2026-09-20)

AC-D1~D9 과 E16~E21 · E23 · E25 는 PASS. **AC-D10 은 FAIL 이었다** — 금지 버튼 검사식 자신이
새고 있었고(2), 살아 있는 페이지마다 비어 있지도 않았다. 검증이 버튼을 심어 검사식을 검사하고,
서버를 띄워 드라이버 라우트의 프로세스 수를 96초 동안 재고, 1280×900 과 390×844 에서 시트의
좌표를 재어 확인했다. 아래 열여섯을 **모두** 이 PR(#170)에서 고쳤다 — 미루는 것은 없다.

앞 절의 남은 것도 여기서 함께 닫았다: §16 의 2 · 5 · 6 · 7 · 9 가 13 · 15 · 14 · 16 · 11 이다.
§16-10(버전 페이지의 «버리기»)은 그대로 소유자 답을 기다린다.

| # | 무엇 | 왜 아픈가 | 한 일 |
|---|---|---|---|
| 1 | **막대가 같은 화면이 부정하는 것을 주장했다** | 묶음의 `upload.json` 이 `status: "success"` 이기만 하면 `pct` 가 **99 로 못 박히고** 근거가 «빌드가 올라갔고 심사 제출만 남았습니다» 였다 — **업로드 잡 자신의 상태는 보지 않았다**. 실기 재현: 업로드 잡이 `failed` 인데 «남은 것» 은 «업로드 #3 실패 — 빌드가 올라가지 않았습니다», 바로 아래 막대는 99 % 에 «빌드가 올라갔고…». `lost`(E20)도, `expired` 행도 같았다. 셋이 함께 틀렸다: 지어낸 숫자 · 분모를 안 대고 조건을 말하는 근거 · 문서만 보고 잡을 안 보는 판정 | **고쳤다.** 숫자는 **선언된** 분모가 있을 때만 낸다(드라이버의 `V S0…S8` 열 칸) — 그때 근거가 «선언된 10 단계» 로 그 분모를 댄다. 드라이버가 없으면 단계 목록은 「지금까지 돈 작업」이라 분모가 자라나므로(첫 작업 하나가 끝나면 1/1 = 100 %) **숫자를 내지 않고** 빗금 막대와 문장만 남긴다. «빌드가 올라갔다» 는 문장은 `upload.json` 과 **업로드 잡의 상태**(`SHEET_UPLOAD_BAD` = lost · failed · timed_out · cancelled)가 둘 다 허락할 때만 나온다 |
| 2 | **금지 버튼 검사가 글자로 면제해서 진짜 게시 버튼이 샜다** | `FORBIDDEN_BUTTONS_JS` 가 `managed[ -]publishing\|관리형 게시` 가 들어간 글을 통째로 버린 뒤에 금지 낱말을 찾았다. 심어서 재어 보니 `Publish now (managed publishing)` · `Managed publishing: Publish to production` · `Start rollout — managed publishing` · `관리형 게시로 지금 게시` 가 **전부 통과**했고(민 `Publish` 는 잡혔다), `Publish to production` 을 심은 pytest 는 빨개지는데 `Publish now (managed publishing)` 을 심은 같은 테스트는 초록이었다. 면제에 쓴 문구가 하필 진짜 게시 버튼이 가장 달기 쉬운 말이다. 게다가 한국어 낱말이 목록에 없어 한국어 버튼은 한 번도 검사받지 않았고, 대기열의 작업 입력 칩(`play_managed_publishing=confirmed-on` — 밑줄이라 면제에도 안 걸렸다)이 걸려 **살아 있는 페이지마다 검사식이 비지 않았다**(AC-D10 이 FAIL 인 이유) | **고쳤다.** 면제를 **신원**으로 바꿨다 — `[data-sheet-fix]`(시트의 «남은 것» 한 줄, 스크롤·포커스만 한다)와 `[data-inputs]`(대기열의 입력 칩, 누르면 JSON 토스트) 둘뿐이다. 낱말에 `게시 · 출시 · 배포 · 롤아웃` 을 더했다. 새 테스트가 열 가지를 심어 **전부** 잡히는지, 면제되는 둘은 같은 글자를 달아도 안 잡히는지 확인한다 |
| 3 | **붙어 있는 시트가 페이지 맨 아래에서 화면 밖으로 밀렸다** | `afterVersionRender` 가 `[data-store-body]` 에 시트 높이만큼 `padding-bottom` 을 줬는데, sticky 의 담는 상자는 부모의 **content box** 라 시트가 그 패딩 안으로 못 내려간다. 1280×900 실측 `sheetTop` −506, 머리 안 보임. 390×844 에서는 시트가 통째로 화면 밖. 펼쳤을 때만이고 접으면 멀쩡하다. 「항상 화면 아래에 붙어 있다」와 AC-E23 의 «접기 버튼 보임» 이 깨진다 | **고쳤다.** 둘이 겹친 문제였다. ① 비워 두는 여백은 시트의 **형제** `.vmain` 이 든다 — 담는 상자가 짧아지지 않는다. ② 시트가 화면보다 크면 `bottom: 0` 으로 붙여도 머리가 위로 밀려 나가므로, 시트 **전체**를 `min(74vh, calc(100vh - 160px))` 로 맞추고 넘치는 것은 본문이 스크롤한다(`.sh-body` 의 70vh · 78vh 상한은 그리로 옮겼다). 두 폭에서 «맨 아래까지 스크롤한 뒤 머리가 화면 안에 통째로 있다» 를 테스트가 잰다 |
| 4 | **드라이버 라우트에 기억이 없다** | `…/release/driver` 를 부를 때마다 빌드 머신에서 대장 `--status` 가 하나씩 돌았다 — 실측 96초에 요청 7 · 프로세스 7(탭 하나당 분당 넷)이고, 탭 수와 사람 수를 곱한다 | **고쳤다.** 소개 자료와 같은 대접 — `--status` **하나만** `DRIVER_STATUS_CACHE_TTL = 20` 초 기억한다. 키는 (저장소 · 대장 체크아웃 sha · build_name · 회차 id · 도는 중인가 · 종료 코드)라 회차가 바뀌거나 끝나면 옛 답은 꺼내지지 않는다 — **낡은 답이 그것이 말하는 것보다 오래 살아남지 못한다**. 답의 나머지(행 · 로그 끝 · plan N)는 매번 새로 읽는다. 웹의 15초 타이머는 그대로다(TTL 을 그보다 길게 잡아야 연이은 두 번이 한 번으로 합쳐진다) |
| 5 | 영어 복수 «1 edited fields» | 한 칸을 고치면 «1 edited fields» · «1 fields differ» 로 나왔다 | **고쳤다** — 셋(`sheet.send.edits` · `version.edit.head_changed` · `version.edit.remote_notice`)이 1 일 때 단수를 쓴다 |
| 6 | «남은 것» 차례가 경고를 막는 것보다 앞에 뒀고, 접힌 머리가 그 경고를 댔다 | `graphics_missing`(경고)이 `listing_unsupported` · `split_unsupported`(막음)보다 앞이라, 접힌 한 줄이 경고를 말하는 동안 막는 것이 그대로 남아 있었다. §4.1 은 차례가 곧 중요도라고 못 박았다 | **고쳤다.** 경고 둘(`graphics_missing` · `driver_no_v`)을 막는 줄 뒤로 옮기고, 접힌 머리는 **첫 막는 줄**을 댄다(막는 것이 없을 때에만 첫 줄) |
| 7 | `failed` 행의 접힌 머리가 «원인 한 줄» 이 아니었다 | §4.3 은 실패한 행이 원인을 한 줄로 말하라고 하는데, 머리는 남은 것의 첫 줄을 댔다 — 행이 들고 있는 `error` 는 어디에도 안 나왔다 | **고쳤다** — `version.state === "failed"` 면 머리는 «실패: <사유>» 다(120자에서 자른다) |
| 8 | 색 사다리에서 도는 회차가 행 자신의 `failed` · `expired` 를 가렸다 | `live` 가 `version.state === "failed"` 와 `expired` 보다 앞이라, 회차가 돌면 실패한 행도 만료된 행도 파랑 «진행 중» 이었다 | **고쳤다** — 행 자신의 끝난 형편이 회차보다 앞이다 |
| 9 | **«제출됨 HH:MM» 이 마지막 편집 시각이었다** | `_sheetSubmittedAt` 이 `release.review.result.finished_at` 을 먼저 봤는데 **서버는 그 열쇠를 보내지 않는다**(`role_entry` 는 `{job_id, state, doc}` 뿐) — 그래서 언제나 `version.last_edit_at` 으로 떨어졌다. 실기: 22:35 에 끝난 심사가 «submitted 01:47» (그때가 마지막 편집). 같은 병이 하나 더 있었다 — Build 행의 «uploaded HH:MM» 도 `release.upload.finished_at` 을 읽어 **한 번도 뜬 적이 없다** | **고쳤다** — 새 `roleFinishedAt(release, entry)` 가 `release.jobs[]` 에서 그 잡 행의 `finished_at` 을 찾는다. 못 찾으면 — 를 그린다. 두 자리가 같은 함수를 쓴다 |
| 10 | `graphics_missing` 의 고치는 곳이 포커스를 못 받았다 | 고치는 곳이 민 `div`(`[data-group="graphics"]`)라 `focusAnchor` 가 스크롤만 하고 포커스는 그대로였다 — 키보드는 따라오지 않는다 | **고쳤다** — 편집 화면의 그룹 상자에 `tabindex="-1"` |
| 11 | «입력한 값은 그대로 둔다» 가 실제보다 넓었다 (§16-9) | 지켜 주는 것은 **사람이 친 칸**뿐이고, 손대지 않은 칸은 원격 값을 칩과 함께 받아들인다 | **고쳤다** — 문구가 그 차이를 말한다(«직접 친 칸은 화면의 글 그대로, 손대지 않은 칸은 그쪽 값을 받았습니다») |
| 12 | **스텁이 서버보다 후해서 9 번을 아무도 못 잡았다** | 노드 테스트와 CDP 스텁이 역할 항목에 `finished_at` 을 실어 줬다 — 서버가 안 보내는 열쇠다. 그래서 그 열쇠를 읽는 코드가 테스트에서는 잘 돌았다 | **고쳤다** — 두 스텁의 역할 항목에서 `finished_at` 을 빼고 `jobs[]` 행으로 옮겼다(스텁은 서버가 **정말 보내는 것**과 같은 모양이어야 한다) |
| 13 | `refusalText` 가 JSON 이 아닌 본문을 안 잘랐다 (§16-2) | HTML 오류 페이지 500자쯤이 저장 배지에 한 줄로 깔렸다. 길이 상한이 없어 어떤 본문이든 화면을 밀어낸다 | **고쳤다** — HTML 로 보이면 **상태 코드만**, 그 밖은 한 줄로 접어 120자에서 자른다 |
| 14 | «파일에서» 가 **계획의** 버전을 읽었다 (§16-6) | `GET …/release/listing` 이 `build_name` 을 받는데 페이지가 안 줘서, 1.2.0 드래프트에 1.1.1 의 릴리스 노트가 «파일에서» 라는 꼬리표를 달고 떴다 | **고쳤다** — 버전 페이지는 상세를 먼저 받고 **드래프트 자신의 이름**을 `build_name` 으로 넘긴다. 다른 드래프트로 가면 그 기억도 버린다 |
| 15 | 한국어 조사 둘 (§16-5) | `version.edit.readonly.closed` 가 «제출됨 라» 로 나왔고, `store.gate.return` 은 으로/로 를 못 골랐다 — `JOSA_PAIRS` 에 그 짝이 없었다 | **고쳤다** — 짝 둘(`이라/라` · `으로/로`)을 더했다. 「으로/로」만 **ㄹ 받침을 받침 없는 쪽**으로 읽으므로(«서울로» · «7로» — 칠) 끝소리 판정을 `none · rieul · other` 셋으로 넓혔다(`finalSound`). 네 쌍의 판정은 그대로다 |
| 16 | «이전 버전과 같음» 스크린샷 칩이 아무것도 재지 않았다 (§16-7) | 서버의 `screenshots` 는 «이전 문안이 그 스토어를 아는가» 일 뿐 파일을 비교하지 않는다 — 프리필에 스크린샷이 아예 없어도 «같음» 이 떴다 | **고쳤다** — 사실만 말한다: «rcm 은 스크린샷을 건드리지 않습니다»(시트의 diff 줄도 같다). 웹 업로드는 이번 범위 밖이다(Q8) |

미루는 것 — **없음**. §16-10(버전 페이지의 «버리기»)만 소유자 답을 기다린다(이 PR 밖).
