...
## 4.7 M5d-3 시작 전 측정 (2026-09-08)

폰·접근성 작업 전에 지금 상태를 먼저 쟀다. **고칠 것을 짐작하지 않기 위해서다.**

### 가로 넘침 — 지금은 없다. 그런데 지키는 게 없다

`?debug=1` 이 쓰는 것과 같은 계산(요소의 `right` 가 뷰포트를 넘는지, `scrollWidth > clientWidth`)을
390 · 1240 두 폭, 한국어·영어 두 언어로 돌렸다. **넷 다 넘침 0.**

| | 390px | 1240px |
|---|---|---|
| 한국어 | ok (390/390) | ok (1240/1240) |
| 영어 | ok (390/390) | ok (1240/1240) |

그러나 이걸 **잠그는 시험이 없다.** M5d-3 의 첫 산출물은 이 네 칸을 시험으로 만드는 것이다.

### 지금 폰 시험은 폰 폭에서 돌지 않는다 (실제 구멍)

`tests/test_web_browser.py::test_mobile_viewport_keeps_queue_content` 는 `window="390,844"` 로
크롬을 띄우고 `assert width <= 720` 을 본다. 그런데 **macOS 의 크롬 창은 500px 밑으로 안 줄어든다** —
실제 뷰포트는 390 이 아니라 **500** 이다. 단언은 통과하지만(500 ≤ 720) 390px 은 한 번도 검사된 적이
없다.

고침: `Emulation.setDeviceMetricsOverride` 로 뷰포트를 덮어쓴다(`tools/screenshots/capture.py` 의
폰 사진이 이미 그렇게 한다). 그러고 나서야 390 에서 무엇이 깨지는지 알 수 있다.

### 대비 — 밝은 판의 상태 필 넷이 미달이었다

필의 글자는 12px 600 이라 WCAG 의 「큰 글자」가 아니다 → **4.5:1** 이 기준이다(1.4.3). 각 상태 색을
자기 `-soft` 배경 위에서 재 보니 밝은 판에서 넷이 모자랐다. 어두운 판은 전부 넘겼다.

| 필 | 고치기 전(밝은 판) | 고친 뒤 | 어두운 판 |
|---|---|---|---|
| running | 4.69 | 4.69 | 5.14 |
| succeeded | **3.79** | 4.53 | 5.99 |
| failed | **4.32** | 4.55 | 4.99 |
| queued · cancelled | **3.83** | 4.56 | 5.55 |
| cancelling | **3.72** | 4.55 | 6.07 |
| lost | 4.53 | 4.53 | 5.78 |

색상은 그대로 두고 밝기만 낮췄다(`--ok` · `--bad` · `--warn` · `--queued`). 의미를 지닌 막대·점은
따로 재서 전부 3:1 을 넘겼다(1.4.11).

### 움직임 — 있어야 할 때 있고, 끄면 없다

`prefers-reduced-motion` 을 CDP 로 바꿔 가며 페이지 전체의 계산된 `animationName` 을 셌다.

| | 애니메이션 도는 요소 |
|---|---|
| `no-preference` | 3 (도는 잡 필의 글리프 둘 + 바쁜 워커 점) |
| `reduce` | **0** |
...
### 대조 결과 (2026-09-08, M5d-3 머지 뒤)

| # | 기준 | 결과 | 근거 |
|---|---|---|---|
| 1 | 한국어로 뜨고 우측 상단에서 영어로. 새로고침·다른 탭 유지 | ✅ | `test_korean_is_the_default_and_the_switch_flips_the_page` |
| 2 | 두 언어에 `undefined`·`NaN`·빈 문자열 없음 | ✅ | `i18n.test.js` 「모든 키가 두 언어에서 비어 있지 않은 문자열을 만든다」 |
| 3 | 이유 칸 `#<id>` 링크가 두 언어에서 동작 | ✅ | `i18n.test.js` 「잡 링크 토큰이 두 언어에 남는다」 |
| 4 | 「내 잡이 끝났나」의 답이 **페이지에서 가장 큰 글자**, 호스트는 접힘 | ✅ | 마지막에 고쳤다 — 답이 13px 로 본문(14)보다 **작았다**. `.sum-yours .v` 20px, 나머지 칸 14px. 호스트는 M5d-2 에서 접었다 |
| 5 | 회색조로 인쇄해도 상태가 구분된다 | ✅ | 모양 + 글자. `state_marks.test.js` 가 아홉 상태의 모양이 서로 다름을 잠근다 |
| 6 | 390px 에서 가로 스크롤 없음 | ✅ | `test_page_never_scrolls_sideways_at_390_and_1240_in_both_languages`. 그전까지 폰 시험은 실제로 500px 에서 돌고 있었다(§4.7) |
| 7 | 기존 테스트 초록 · 스키마 v1 · 런타임 의존성 0 | ✅ | 매 PR 의 CI |

**4번이 마지막까지 안 맞았다.** 타이포를 산세리프로 옮기면서도 크기 단계(12/14/16/20/28)는 쓰지
않아 페이지 전체가 11~15px 에 몰려 있었다. 위계를 굵기와 색으로만 주면 「가장 자주 받는 질문」이
가장 크게 보이지 않는다 — 크기도 채널이다.
